...
## Task 4: La fase `Voting` nel motore

Il task più grosso del lotto, e volutamente indivisibile: l'ingresso nella fase
senza il voto lascerebbe una partita che non finisce più, e dei test disattivati
in attesa del task successivo. Un task deve lasciare software che funziona.
...
- Produces:
  ...
  - `record VoteCast(Guid PlayerId, int PhraseIndex) : GameEvent`
  - `record VotingCloseRequested(Guid RequestedBy) : GameEvent`
  - `GameEngine.VotantiAttesi(GameState)`, `GameEngine.TuttiHannoVotato(GameState)`, `GameEngine.Avanzamento(GameState)`, `GameEngine.EntraInVoto(...)`, `GameEngine.ChiudiVoto(...)` — tutti `private static`.

- [ ] **Step 1: Scrivere i test dell'ingresso che falliscono**
...
> A questo punto la partita entra nella fase di voto ma nessuno può ancora
> votare: `RevealTests` è rosso e la suite completa **non** va eseguita finché
> il voto non è implementato. È il motivo per cui questo task non si divide.

- [ ] **Step 9: Scrivere i test del voto che falliscono**
...
- [ ] **Step 10: Eseguire per verificare che fallisca**
...
- [ ] **Step 12: Registrarli nel dispatch**
...
- [ ] **Step 14: Aggiornare i test del reveal**

Il reveal non chiude più la partita, quindi `RevealTests.ScopertaLUltimaFraseLaPartitaEConclusa` e `GliAutoriArrivanoSoloConIlMessaggioFinale` (scritti nel Task 3) sono rossi. In `tests/FrasiSquisite.Domain.Tests/Engine/RevealTests.cs` vanno sostituiti da un solo test che porta fino in fondo: dopo il ciclo di reveal, far votare tutti e leggere il messaggio finale dall'ultimo risultato.
...
Cancellare sia il vecchio `ScopertaLUltimaFraseLaPartitaEConclusa` sia `GliAutoriArrivanoSoloConIlMessaggioFinale`: entrambi sono coperti da questo.

- [ ] **Step 15: Eseguire la suite completa**

```bash
dotnet test FrasiSquisite.slnx --nologo -v q
```

Atteso: `Non superati: 0` su tutti e quattro i progetti. È la prima esecuzione completa da quando il reveal ha smesso di chiudere la partita: qui si vede se è rimasto qualcosa in sospeso.

- [ ] **Step 16: Commit**

```bash
git add -A src tests && git commit -m "feat(voto): fase di voto nel motore, un voto a testa con classifica finale"
```

---

## Task 5: Chiusura forzata dall'host e popolazione che cambia
...
**Interfaces:**
- Consumes: `TuttiHannoVotato`, `ChiudiVoto` e `VotingCloseRequested` (Task 4).
...
## Task 6: L'hub — `CastVote` e `CloseVoting`
...
**Interfaces:**
- Consumes: `VoteCast`, `VotingCloseRequested` (Task 4).
- Produces:
...
**Interfaces:**
- Consumes: `CastVoteRequest`, `CloseVotingRequest` (Task 6).
- Produces:
...
## Task 8: Il client — ViewModel della fase di voto
...
**Interfaces:**
- Consumes: `CastVoteAsync`/`CloseVotingAsync` (Task 7), `VoteRequestMessage`/`VoteProgressMessage` (Task 4).
...
## Task 10: Verifica sul telefono e chiusura del lotto
